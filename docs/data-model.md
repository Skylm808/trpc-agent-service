...
| `audit_logs` | `tenant_id`, `audit_id`, `channel`, `user_id`, `session_id`, `agent_name`, `tool_name`, `decision`, `latency_ms`, `error_type`, `cost_micros`, `trace_id` | PK `(tenant_id, audit_id)`；按 tenant/trace/time 查询；`cost_micros` 由请求固定的模型价格版本和 provider usage 核销 |
