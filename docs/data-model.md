...
| `audit_logs` | `tenant_id`, `audit_id`, `channel`, `user_id`, `session_id`, `agent_name`, `tool_name`, `decision`, `latency_ms`, `error_type`, `cost_micros`, `trace_id` | PK `(tenant_id, audit_id)`；按 tenant/trace/time 查询；精确成本接入前 `cost_micros` 为 0 |
