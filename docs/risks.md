...
| R9 | 模型 API key、IM token、数据库密码出现在配置、错误或 trace 中 | 凭据泄露，攻击者跨租户调用外部服务 | 配置只保存 SecretRef；运行时支持 env/file、Vault KV v2-compatible 与 KMS-compatible HTTPS Resolver；bootstrap token 由部署 Secret 注入；结构化字段和 Runner Event 双层脱敏；外部 Provider 错误不带 endpoint/key/body | CI 扫描 secret pattern；用 canary secret 跑日志/trace/Plugin 泄漏测试 |
